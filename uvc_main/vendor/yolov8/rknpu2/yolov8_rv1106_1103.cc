...
{
    APP_LOGD("  index=%d, name=%s, n_dims=%d, dims=[%d, %d, %d, %d], n_elems=%d, size=%d, fmt=%s, type=%s, qnt_type=%s, "
           "zp=%d, scale=%f\n",
...
int init_yolov8_model(const char *model_path, rknn_app_context_t *app_ctx)
{
    ...
    if (ret != RKNN_SUCC)
    {
        APP_LOGE("rknn_query fail! ret=%d\n", ret);
        return -1;
    }
    APP_LOGI("[yolov8] model input num: %d, output num: %d\n", io_num.n_input, io_num.n_output);

    // Get Model Input Info
    APP_LOGD("[yolov8] input tensors:\n");
    ...
    APP_LOGD("[yolov8] output tensors:\n");
    ...
    APP_LOGD("[yolov8] input_attrs[0].size_with_stride=%d\n", input_attrs[0].size_with_stride);
    ...
    if (ret < 0) {
        APP_LOGE("input_mems rknn_set_io_mem fail! ret=%d\n", ret);
        return -1;
    }
    ...
    for (uint32_t i = 0; i < io_num.n_output; ++i) {
        ...
        if (ret < 0) {
            APP_LOGE("output_mems rknn_set_io_mem fail! ret=%d\n", ret);
            return -1;
        }
    }
...
int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results)
{
    ...
    if (ret < 0)
    {
        APP_LOGE("convert_image_with_letterbox fail! ret=%d\n", ret);
        return -1;
    }

    // Run
    APP_LOGD("[yolov8] rknn_run\n");
