...
static int readLines(const char *fileName, char *lines[], int max_line)
{
    ...
    if (file == NULL)
    {
        APP_LOGE("[yolov8 post] Open %s fail!\n", fileName);
        return -1;
    }
...
{
    APP_LOGI("[yolov8 post] load label %s\n", locationFilename);
...
static int process_i8_rv1106(int8_t *box_tensor, int32_t box_zp, float box_scale,
    ...
    APP_LOGD("[yolov8 post] validCount=%d\n", validCount);
    APP_LOGD("[yolov8 post] grid h-%d, w-%d, stride %d\n", grid_h, grid_w, stride);
    return validCount;
}
#endif
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            APP_LOGE("[yolov8 post] RV1106/1103 only support quantization mode\n");
            return -1;
        }
...
int init_post_process(const char* labels_path)
{
    int ret = 0;
    const char* path = labels_path ? labels_path : LABEL_NALE_TXT_PATH;
    ret = loadLabelName(path, labels);
    if (ret < 0)
    {
        APP_LOGE("[yolov8 post] Load %s failed!\n", path);
        return -1;
    }
    return 0;
}
