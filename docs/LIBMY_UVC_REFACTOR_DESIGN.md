...
2. **`uvctest`**：仅作 **本仓库内的测试/示例程序**——用于验证 **`libmy_uvc` + `libmy_uvc_pip_helper`** 的联调行为；**所有** 从磁盘/目录读取的媒体（**含** H.264、YUV、JPEG 及类似 `pip_overlay_path` 的路径）**均在 `uvctest` 内完成解码/读帧**，再经 **API** 把「基础画布 + 各路 PiP 画面」以**数据流/缓冲区**形式交给 `pip_helper`，**不作为**第三方集成时的必选能力。
3. **`libmy_uvc_pip_helper.a`**（方案 B）：**协议栈外** 完成 **全画布合成**——**独立主讲人 PiP** + **置于最底层的全屏网格**（布局 **`n_tiles`（1～16）**，每帧 **`n_active`（0～`n_tiles`）** 仅叠前 **`n_active` 路**，余格停止叠加该路，见 §2.4）；**主讲人 / 各路 tile** 在 **无新数据时沿用库内上一帧**，超过 **配置的超时时间**（**缺省 5000 ms**，见 **`[libmy_uvc_pip]`** 与 **`pip_helper_config_t`**）仍无新数据则 **停止叠加该路**（§2.4）；**v1 合成 API 仅接收 NV12**（基础画布每帧；主讲人与 tile 按 API 更新或冻结）；合成结果（MJPEG，与头文件一致）再交给 **`my_uvc_submit_mjpeg`**。**面向第三方交付**：第三方在应用内将 H.264/MJPEG/YUV 等 **解码并缩放到约定尺寸后** 再传入，**不得依赖** `pip_helper` 内读路径或接受压缩帧作为 v1 入参（见 §3.8、§2.5）。
...
| **`libmy_uvc_pip_helper.a`** | **每帧**接收调用方给出的 **基础画布 + 主讲人 + 全屏 N 路** 的 **NV12** 缓冲区（v1）；完成 RGA/MPP 等合成；**输出 MJPEG**；**不**链接 `libmy_uvc.so`；**不**承担产品级文件路径读取（§2.5、§3.8）。 |
...
- **画面结构**：目前包含两层叠加关系。
  - **底层（全屏网格）**：取消了独立的静态背图。底部网格被修改为最底层，高度占全屏高度。全屏画布内按 **`n_tiles`（1≤`n_tiles`≤16）** 计算多路视频的格子几何布局（如 **≤4** 路占满单行、**>4** 路分双行等，**间隙/边距可配**），算法与 **`pip_tile_layout.hpp`** 的 `pip_tile_layout_full_screen` 一致。
  - **顶层（主讲人/VI主画面）**：整个摄像头输入（VI画面）目前由原主讲人窗口来显示，作为浮于全屏网格之上的独立顶层窗口（矩形位置和尺寸可配置，对应原 `pip_x/y/w/h` 语义）。
- **每帧有效网格路数 `n_active`（v1 定案，选 A）**：**每帧** 合成调用携带 **`n_active`（0≤`n_active`≤`n_tiles`，名称以 `pip_helper.h` 为准）**。仅 **布局顺序下前 `n_active` 个格子** 叠对应 **tile NV12**；**其余格子不叠图**，该区域 **保持空白/透明**（不读占位缓冲、不要求调用方提供无效路的 NV12）。**`n_active` 可帧间变化**。`n_active=0` 表示本帧网格 **全部停止叠加**（**主讲人/VI画面** 仍按配置单独叠加）。**布局上限 `n_tiles`** 可在 create 时或配置中给定，与 **`pip_tile_layout`** 的 `n_tiles` 一致。
- **时间对齐**：**基础画布**（空底图）每帧与合成调用 **同一时刻** 提交；**主讲人/VI画面** 与 **各路 tile** 按 §2.4 下条 **「间断策略」** 与 API 约定提交新帧或沿用缓存（细则见 §8）。
- **数据源间断策略（v1 定案）**：**主讲人** 与 **下排每一参与合成的 grid 路**（对 **前 `n_active` 个槽位**，各路流 **相互独立**）——  
  ...
  - **自该路最后一次收到有效数据起，连续超过配置时长仍无新数据**：该路 **不再叠加**，**对应区域停止叠加该路**（主讲人矩形或该 grid 格），直至该路 **再次出现有效数据**。  
  ...
  - **`pip_helper_create` 未显式自定义超时（v1 定案，问题 6 选 A）**：**与 ini 缺省一致，库内按 5000 ms 生效**。**C 结构体推荐** 使用 **有符号整数**（如 **`int32_t`**）：**`-1`**（或宏 **`PIP_OVERLAY_STALE_TIMEOUT_USE_DEFAULT`**）表示 **「使用库缺省 5000」**；**`0`** 表示 **不因超时停止叠加该路**（**仅冻结上一帧**）；**`>0`** 为显式毫秒。**注意**：若将结构体 **`memset` 成全零** 且本字段为 **`0`**，语义为 **禁用超时**，**不是** 缺省 5000；需要缺省时请置 **`-1`** 或使用文档提供的 **初始化宏**。  
  ...
  - **基础画布**：本条 **不**约束基础画布源；基础画布仍按 **每帧由调用方提供**（若无基础画布则另属错误/重试语义，不在此扩展）。
- **输入形态（交付，v1 定案）**：**仅 NV12**。**基础画布、主讲人、每路 tile** 均为 **NV12**（含 **width/height/stride** 等描述，以 **`pip_helper.h`** 为准）。H.264、MJPEG、其它 YUV 等 **一律在应用内** 解码、必要时 **缩放/裁剪** 到与布局一致的平面尺寸后再传入；**v1 不接受** JPEG/H.264 **压缩帧** 作为合成输入（减少库内二次解码与格式分支，利于 RGA/MPP 快路径）。若未来扩展其它像素格式或「压缩入参」，须 **新版本 API** 与文档修订。
...
| 调用 `pip_helper` | 是，传入 **基础画布 + 主讲人 + N 路** **NV12** 缓冲区 | 同上 |
...
- **多路**：各路基础画布、网格、主讲人状态 **相互隔离**；资源占用（RGA/MPP）由实现按需创建，**v1 不**引入跨通道共享单例。

### 3.8 PiP：配置、ini 与运行时 API（**定稿**）

**v1 像素格式（已定案）**：**`pip_helper` 合成接口只接受 NV12**。基础画布须为 **与画布同宽高** 的 NV12（或与头文件约定的对齐方式）；**主讲人** 与 **每路 grid tile** 为 **各自 NV12 平面**，尺寸与布局格一致或由 API 描述缩放目标（以 `pip_helper.h` 为准）。**UVC 侧为 MJPEG** 时：应用先 **解码 MJPEG → NV12**，合成后 **`pip_helper` 输出 MJPEG** 再 `my_uvc_submit_mjpeg`，整条链在像素域以 NV12 为主，与 RGA/MPP 常见快路径一致。

**v1 数据源间断（已定案）**：**主讲人** 与 **各路 grid** 在 **无新帧** 时 **沿用库内缓存的上一帧**；超过 **有效超时毫秒数**（**create 时** 由 **`pip_overlay_stale_timeout_ms`** 决定：**`-1`/缺省 → 5000**；**`0` → 禁用超时仅冻结**；**`>0` → 自定义**，见 §2.4）仍无该路新有效数据则 **停止叠加该路**。API 须能表达 **本帧某路是否提交新 NV12**（或等价约定），且 **`pip_helper` 在更新时拷贝** 以满足与 **§3.3** 一致的缓冲生命周期。
...
1. **第三方集成（目标）**  
   ...
   - **每帧合成调用**：传入 **基础画布**；**主讲人** 与各 tile 路按 API 提交 **新 NV12** 或 **本帧无更新**（触发 §2.4 冻结/超时逻辑）；**本帧 `n_active`** 与 **前 `n_active` 格** 的语义仍适用（**`n_active`≤`n_tiles`**，**余格停止叠加该路**）。**不**通过 `pip_overlay_path` 由库内读盘完成产品功能；**v1 不入参压缩 JPEG/H.264**。

2. **`uvctest` / `[libmy_uvc_pip]`（验证与示例）**  
   - **`pip_overlay_path`**、**`[uvctest]`** 下各通道媒体路径等：仅表示 **测试程序** 从哪里 **读文件**；读入后 **解码为 NV12**（及缩放），再拆成基础画布 / 主讲人 / N 路 **调用 `pip_helper`**。  
...
2. 实现 §2.4 / §3.8：**主讲人 + 全屏网格（`n_tiles` + 每帧 `n_active`）**、**每帧 API 仅入参 NV12（基础画布/主讲人/前 `n_active` 路 tile）**、**MJPEG 输出**；**库内不以路径为产品接口**（读盘仅保留在 `uvctest` 或过渡期，须标注 deprecated）。
...
**验收**：PiP 开/关、**`n_active` 变更**、多路 tile、**断流冻结 / 超时停止叠加该路**（**`pip_overlay_stale_timeout_ms` 缺省与自定义、`0` 若支持**）、与 `libmy_uvc` 组合无死锁、无双重释放；第三方集成路径 **不依赖** `pip_helper` 打开媒体文件。
...
- **PiP**：**基础画布** 与 **每帧合成** 对齐；**主讲人 / tile** 支持 **断流冻结上一帧** 与 **可配置超时停止叠加该路**（§2.4），须验证 **单调时钟**、多路 **独立超时**、**`pip_overlay_stale_timeout_ms`** 与 **`n_active`** 组合无错乱；**`pip_helper` 更新层时拷贝 NV12** 与 **§3.3** 缓冲生命周期一致。**方案 B** 下应用负责 **同时链接** 两库、**应用侧读盘与解码至 NV12** 与 **`pip_helper` → `libmy_uvc` 送帧顺序**。
...
| **v1.7** | 2026-04-21 | **PiP 交付边界**：§1.2/§2.2/§2.3/§2.4/§2.5 明确 **第三方仅收两库 + API**；**路径/H.264/YUV/JPEG 读盘与解码均在应用（`uvctest` 为示例）**；**主讲人独立 + 全屏动态 N 路（≤16）** 经 **每帧缓冲区** 传入 `pip_helper`；重写 **§3.8**；§7 阶段 3、§8 PiP 风险对齐 |
| **v1.8** | 2026-04-21 | **v1 像素格式定案**：**`pip_helper` 合成 API 仅接收 NV12**（基础画布/主讲人/tile）；**不接受压缩帧入参**；§1.2、§2.3–§2.5、§3.8 首部、§7 阶段 3、§8、文末结论对齐 |
| **v1.9** | 2026-04-21 | **网格有效路数**：每帧 **`n_active`（≤`n_tiles`）**，仅前 **`n_active`** 格叠 tile，**余格停止叠加该路**；不要求无效路 NV12；§2.4、§3.8、§8 对齐 |
| **v1.10** | 2026-04-21 | **断流策略**：**主讲人 + 各 grid 路** 无新数据时 **沿用库内上一帧**；超时后 **停止叠加该路**；`pip_helper` **拷贝**缓存、单调时钟；§2.4、§3.8、§7 阶段 3 验收、§8 |
| **v1.11** | 2026-04-21 | **断流超时可配置（选 C）**：**`pip_overlay_stale_timeout_ms`** 在 **`[libmy_uvc_pip]`** 与 **`pip_helper_config_t`/`pip_helper_create`** 同步，**缺省 5000**；**`0` = 不因超时停止叠加该路**（仅冻结）；§1.2、§1.3、`libmy_uvc_pip.ini`、§2.4、§3.8、§7、§8、结论 |
